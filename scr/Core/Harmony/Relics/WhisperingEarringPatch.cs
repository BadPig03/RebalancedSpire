...
public static class WhisperingEarringPatch
{
    ...
    [HarmonyPatch(typeof(WhisperingEarring), nameof(WhisperingEarring.CanonicalVars), MethodType.Getter)]
    [HarmonyPrefix]
    [UsedImplicitly]
    ...
    [HarmonyPrefix]
    [UsedImplicitly]
    private static bool PreFix_BeforePlayPhaseStartLate(WhisperingEarring __instance, PlayerChoiceContext choiceContext, Player player, ref Task __result)
    {
        if (Disabled)
        {
            return true;
