...
public static class CrossbowPatch
{
    ...
    private static async Task AfterSideTurnStart(Crossbow instance)
    {
        ...
        await CardPileCmd.AddGeneratedCardsToCombat(chosenCards, PileType.Hand, false);
    }
    ...
    [HarmonyPrefix]
    [UsedImplicitly]
    private static bool PreFix_AfterSideTurnStart(Crossbow __instance, CombatSide side, CombatState combatState, ref Task __result)
    {
        if (Disabled)
        {
            return true;
