...
{
    private static readonly bool Disabled = !RebalancedSpireSettingsStore.Settings.OrobasAncientChoices;

    private static List<EventOption> OptionPool1(Orobas instance) =>
    [
        SrcHelpers.RelicOption<Driftwood>(instance),
        SrcHelpers.RelicOption<GlassEye>(instance),
        SrcHelpers.RelicOption<SandCastle>(instance)
    ];

    private static List<EventOption> OptionPool2(Orobas instance) =>
    [
        SrcHelpers.RelicOption<AlchemicalCoffer>(instance),
        SrcHelpers.RelicOption<ElectricShrymp>(instance),
        SrcHelpers.RelicOption<RadiantPearl>(instance)
    ];
    ...
    private static List<EventOption> OptionPool3(Orobas instance)
    {
        ...
        {
            list.Add(SrcHelpers.RelicOption(instance, touchOfOrobas));
        }
        ...
        {
            list.Add(SrcHelpers.RelicOption(instance, archaicTooth));
        }
        if (list.Count == 0)
        {
    ...
    private static List<EventOption> SeaGlasses(Orobas instance)
    {
        ...
        foreach (CharacterModel allCharacter in ModelDb.AllCharacters)
        {
            ...
            list.Add(SrcHelpers.RelicOption(instance, seaGlass));
        }
        return list;
    }

    private static EventOption PrismaticGem(Orobas instance) => SrcHelpers.RelicOption<PrismaticGem>(instance);
    ...
    private static bool PreFix_GenerateInitialOptions(Orobas __instance, ref IReadOnlyList<EventOption> __result)
    {
        ...
        else
        {
            ...
            list.Add(SrcHelpers.RelicOption(__instance, seaGlass));
        }
