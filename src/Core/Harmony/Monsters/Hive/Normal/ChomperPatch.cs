...
public static class ChomperPatch
{
    ...
    {
        await DamageCmd.Attack(ClampDamage).WithHitCount(ClampCount).FromMonster(instance).WithAttackerAnim("Attack", 0.3f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")!).WithHitFx("vfx/vfx_attack_slash").Execute(null);
    }
    ...
    [HarmonyPatch(typeof(Chomper), "GenerateMoveStateMachine")]
    [HarmonyPrefix]
    [UsedImplicitly]
