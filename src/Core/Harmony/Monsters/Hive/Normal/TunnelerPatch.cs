...
public static class TunnelerPatch
{
    ...
    {
        await DamageCmd.Attack(instance.BelowDamage).FromMonster(instance).WithAttackerAnim("Attack", 0.25f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitFx("vfx/vfx_attack_slash").Execute(null);
    }
    ...
    [HarmonyPatch(typeof(Tunneler), "GenerateMoveStateMachine")]
    [HarmonyPrefix]
    [UsedImplicitly]
    private static bool PreFix(Tunneler __instance, ref MonsterMoveStateMachine __result)
    {
        ...
        MoveState moveState2 = new MoveState("BURROW_MOVE", __instance.BurrowMove, new BuffIntent(), new DefendIntent());
        MoveState moveState3 = new MoveState("BELOW_MOVE", t => BelowMove(__instance, t), new SingleAttackIntent(__instance.BiteDamage));
        MoveState moveState4 = new MoveState("DIZZY_MOVE", __instance.StillDizzyMove, new StunIntent());
