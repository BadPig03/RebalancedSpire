...
public static class HunterKillerPatch
{
    ...
    {
        await DamageCmd.Attack(BiteDamage).FromMonster(instance).WithAttackerAnim("Attack", 0.3f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitFx("vfx/vfx_bite").Execute(null);
    }

    private static async Task PunctureMove(HunterKiller instance)
    {
        await DamageCmd.Attack(PunctureDamage).WithHitCount(PunctureCount).OnlyPlayAnimOnce().FromMonster(instance).WithAttackerAnim("TripleAttack", 0.3f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitFx("vfx/vfx_attack_slash").Execute(null);
    }
    ...
    [HarmonyPatch(typeof(HunterKiller), "GenerateMoveStateMachine")]
    [HarmonyPrefix]
    [UsedImplicitly]
