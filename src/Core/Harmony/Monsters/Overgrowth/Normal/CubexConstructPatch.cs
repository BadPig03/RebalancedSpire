...
public static class CubexConstructPatch
{
    ...
    private static int BlastDamage => AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 10, 9);
    ...
    private static async Task RepeaterBlastMove(CubexConstruct instance)
    {
        ...
        await Cmd.Wait(0.4f);
        await DamageCmd.Attack(BlastDamage).FromMonster(instance).WithAttackerAnim("Attack", 0f).WithHitFx("vfx/vfx_attack_blunt", null, "blunt_attack.mp3").Execute(null);
    ...
    private static bool PreFix_GenerateMoveStateMachine(CubexConstruct __instance, ref MonsterMoveStateMachine __result)
    {
        ...
        MoveState moveState2 = new MoveState("REPEATER_BLAST_MOVE", _ => RepeaterBlastMove(__instance), new SingleAttackIntent(BlastDamage));
        MoveState moveState3 = new MoveState("REPEATER_BLAST_MOVE_2", _ => RepeaterBlastMove(__instance), new SingleAttackIntent(BlastDamage));
