...
public static class ByrdonisPatch
{
    ...
    {
        await DamageCmd.Attack(Byrdonis.PeckDamage).WithHitCount(Byrdonis.PeckRepeat).FromMonster(instance).WithAttackerAnim("AttackAngry", 0.4f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitFx("vfx/vfx_attack_slash").Execute(null);
    }

    private static async Task SwoopMove(Byrdonis instance)
    {
        await DamageCmd.Attack(SwoopDamage).FromMonster(instance).WithAttackerAnim("AttackAngry", 0.4f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitFx("vfx/vfx_attack_slash").Execute(null);
    }
    ...
    [HarmonyPatch(typeof(Byrdonis), "AfterAddedToRoom")]
    [HarmonyPrefix]
    [UsedImplicitly]
