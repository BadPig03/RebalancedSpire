...
public static class PhrogParasitePatch
{
    ...
    private static int MinInitialHp => AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 75, 67);
    ...
    {
        await DamageCmd.Attack(LashDamage).WithHitCount(GetInfestedAmount(instance)).FromMonster(instance).WithAttackerAnim("Attack", 0.55f).OnlyPlayAnimOnce().WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitVfxNode(NWormyImpactVfx.Create).Execute(null);
    }
    ...
    [HarmonyPatch(typeof(PhrogParasite), "MinInitialHp", MethodType.Getter)]
    [HarmonyPostfix]
    [UsedImplicitly]
    ...
    [HarmonyPatch(typeof(PhrogParasite), "AfterAddedToRoom")]
    [HarmonyPrefix]
    [UsedImplicitly]
    ...
    [HarmonyPatch(typeof(PhrogParasite), "GenerateMoveStateMachine")]
    [HarmonyPrefix]
    [UsedImplicitly]
