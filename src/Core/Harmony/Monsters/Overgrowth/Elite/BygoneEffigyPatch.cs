...
public static class BygoneEffigyPatch
{
    ...
    private static async Task SlashMove(BygoneEffigy instance, IReadOnlyList<Creature> targets)
    {
        ...
        await DamageCmd.Attack(instance.SlashDamage).FromMonster(instance).WithAttackerAnim("Attack", 0.1f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitFx("vfx/vfx_attack_slash").Execute(null);
        await PowerCmd.Apply<StrengthPower>(new ThrowingPlayerChoiceContext(), instance.Creature, StrengthPowerAmount, instance.Creature, null);
        await Cmd.Wait(0.25f);
    }

    [HarmonyPatch(typeof(BygoneEffigy), "GenerateMoveStateMachine")]
    [HarmonyPrefix]
    [UsedImplicitly]
