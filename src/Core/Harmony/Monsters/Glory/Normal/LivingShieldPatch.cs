...
public static class LivingShieldPatch
{
    ...
    {
        await DamageCmd.Attack(SmashDamage).FromMonster(instance).WithAttackerAnim("Attack", 0.3f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitFx("vfx/vfx_attack_slash").Execute(null);
    ...
    [HarmonyPatch(typeof(LivingShield), "AfterAddedToRoom")]
    [HarmonyPrefix]
    [UsedImplicitly]
    ...
    [HarmonyPatch(typeof(LivingShield), "GenerateMoveStateMachine")]
    [HarmonyPrefix]
    [UsedImplicitly]
