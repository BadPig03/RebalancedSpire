...
public static class SoulNexusPatch
{
    ...
    {
        await DamageCmd.Attack(SoulStrikeDamage).WithHitCount(SoulStrikeRepeat).FromMonster(instance).OnlyPlayAnimOnce().WithAttackerAnim("Attack", 0.8f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitFx("vfx/vfx_attack_slash").Execute(null);
    }

    private static async Task MaelstromMove(SoulNexus instance)
    {
        await DamageCmd.Attack(MaelstromDamage).WithHitCount(MaelstromRepeat).FromMonster(instance).OnlyPlayAnimOnce().WithAttackerAnim("Attack", 0.8f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitFx("vfx/vfx_attack_slash").Execute(null);
    }

    private static async Task DrainLifeMove(SoulNexus instance)
    {
        await DamageCmd.Attack(DrainLifeDamage).FromMonster(instance).WithAttackerAnim("Attack", 0.6f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitFx("vfx/vfx_attack_slash").Execute(null);
    }
    ...
    {
        await DamageCmd.Attack(instance.SoulBurnDamage).FromMonster(instance).WithAttackerAnim("Attack", 0.6f).WithAttackerFx(null, SrcHelpers.GetSfx(instance, "AttackSfx")).WithHitFx("vfx/vfx_attack_slash").Execute(null);
    }
    ...
    [HarmonyPatch(typeof(SoulNexus), "GenerateMoveStateMachine")]
    [HarmonyPrefix]
    [UsedImplicitly]
